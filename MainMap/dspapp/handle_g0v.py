...
#         self.data_filename = "g0v_data.pickle"
        ...
        self.data_filename = os.path.join(path_root,self.data_filename)
        try:
...
#                 self.g0v_data = marshal.load(op)
    ...
    def read_pos_map3(self):
        return {
            ...
            u'苗栗縣頭份市': [24.6666839,120.8541887],
            u'彰化縣員林市': [23.9568477,120.5557177],            
        }
    ...
    def transform(self):
        ...
        for filename, enc in self.filenames:
            data = pd.read_csv(os.path.join(self.path_root,filename), encoding=enc)
...
if __name__ == '__main__':
    ...
    print "Module Load Run time: {}".format(time.time()-st)
    # print g0v_data.pos_map(u'金門縣金沙鎮')
    # tmp = [ d for d in g0v_data.g0v_data if u'登革熱' in d['category'] ]
    # print tmp[0]
    
    st = time.time()
    a = g0v_data.get_data([u"流感",u"登革熱"])
    print "Module Query Run time: {}".format(time.time()-st)
    print len(a)
    print a[0]